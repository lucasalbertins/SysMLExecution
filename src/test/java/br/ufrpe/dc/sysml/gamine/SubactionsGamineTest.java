...
public class SubactionsGamineTest {
    ...
    static void init() {
        ...
        spec.parseFile("control/SubactionDecisionNodeExample.sysml");
    ...
    @Test
    void testSubactionTracking() {
        var semantics = createSemantics("machine");
        ...
        var result = sequencer.run(c -> {
            // Print active subactions at each step
            if (!c.activeSubactions.isEmpty()) {
                System.out.println("Active Subactions: " + c.activeSubactions.keySet());
            }
            System.out.println("Memory State: " + c.memory);
            System.out.println("Configuration: " + c);
            return count[0]-- <= 1;
        });
        
        System.out.println("Execution completed with result: " + result);
    }
//    @Test
//    void newTest() {
